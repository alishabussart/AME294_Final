...
function formatRSS(data) {
   ...
   for(var i = 0; i < rssFeed.length; i++) {
       ...
       output = output + "<div class='category'>" + rssFeed[i].category.attributes.label.toLowerCase() + "</div>"
       output = output + "<hr>";
       output = output + "</div>";
   }
...
function listFilter() {
    var filter = $.map($("#catSel").toArray(), function(e){
        return $(e).val().toLowerCase();
    });
    if (filter == "all") {
        $("#list").find(".list-item").slideDown();
        itemSelected(0);
    } else {
        $("#list div").removeClass("selected");
        $("#list").find("div.category:not(:contains(" + filter + "))").parent().slideUp();
        $("#list").find("div.category:contains(" + filter + ")").parent().slideDown();
